...
export async function handleConfirmarColeta(message: string) {
  ...
  if (removeColeta) {
    console.log("✅ Publicando no tópico:", `user/confirmarColetaResponse/${requestId}`);
    console.log("Client connected: ", client.connected)
    const response = { message: "Coleta confirmada com sucesso" };
   client.publish(
